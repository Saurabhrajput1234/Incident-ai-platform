...
import { AlertCircle, LayoutDashboard, Plus, CalendarDays, BarChart2, Upload } from 'lucide-react'
      ...
      <header className="bg-gray-900 text-white px-6 py-3 flex items-center justify-between shadow-md">
        ...
        <nav className="flex items-center gap-3 text-sm">
          <NavLink to="/dashboard" className={({ isActive }) =>
            `flex items-center gap-1.5 px-3 py-1.5 rounded transition-colors ${isActive ? 'bg-blue-600 text-white' : 'text-gray-300 hover:text-white hover:bg-gray-700'}`}>
            <BarChart2 size={15} /> Dashboard
          </NavLink>
          <NavLink to="/incidents" className={({ isActive }) =>
            `flex items-center gap-1.5 px-3 py-1.5 rounded transition-colors ${isActive ? 'bg-blue-600 text-white' : 'text-gray-300 hover:text-white hover:bg-gray-700'}`}>
            <LayoutDashboard size={15} /> Incidents
          </NavLink>
          <NavLink to="/shift-roster" className={({ isActive }) =>
            `flex items-center gap-1.5 px-3 py-1.5 rounded transition-colors ${isActive ? 'bg-blue-600 text-white' : 'text-gray-300 hover:text-white hover:bg-gray-700'}`}>
            <CalendarDays size={15} /> Shift Roster
          </NavLink>
          <NavLink to="/bulk-import" className={({ isActive }) =>
            `flex items-center gap-1.5 px-3 py-1.5 rounded transition-colors ${isActive ? 'bg-blue-600 text-white' : 'text-gray-300 hover:text-white hover:bg-gray-700'}`}>
